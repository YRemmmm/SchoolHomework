...
#  endif
   ...
   for (step = 1; step <= n_steps; step++) {
      t = step*delta_t;
      memset(buffer, 0, n*n*sizeof(double));
      ...
      for (part = 0; part < n; part++) {
         ...
         double curr_part_s_X, curr_part_s_Y, curr_part_m;

         curr_part_s_X = curr[part].s[X];
         curr_part_s_Y = curr[part].s[Y];
         curr_part_m = curr[part].m;
         for (k = part + 1; k < n; k++) {
            /* Compute force on part due to k */
            f_part_k_X = curr_part_s_X - curr[k].s[X];
            f_part_k_Y = curr_part_s_Y - curr[k].s[Y];
            len = sqrt(f_part_k_X*f_part_k_X + f_part_k_Y*f_part_k_Y);
            len_3 = len*len*len;
            mg = -G*curr_part_m*curr[k].m;
            fact = mg/len_3;
            buffer[k*n+part] = fact;
         }
      }
      # pragma omp parallel for schedule(dynamic)
      for (part = 0; part < n; part++) {
         ...
         double f_part_k_X, f_part_k_Y;
         double sum_X, sum_Y;
         double curr_part_s_X, curr_part_s_Y;
         ...
         for (k = 0; k < part; k++) {
            ...
            sum_X += f_part_k_X * buffer[part*n+k];
            sum_Y += f_part_k_Y * buffer[part*n+k];
         }
         for (k = part + 1; k < n; k++) {
            ...
            f_part_k_Y = curr_part_s_Y - curr[k].s[Y];
            /* Add force in to total forces */
            sum_X += f_part_k_X * buffer[k*n+part];
            sum_Y += f_part_k_Y * buffer[k*n+part];
         }
