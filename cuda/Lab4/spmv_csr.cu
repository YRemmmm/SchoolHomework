...
void cpu_spmv_csr(int N, int *rowPtr, int *colIndex, float *values, float *vector, float *result)
{
    for (int row = 0; row < N; ++row)
    {
        result[row] = 0.0f;
        for (int col = rowPtr[row]; col < rowPtr[row + 1]; ++col)
        {
            result[row] += values[col] * vector[colIndex[col]];
        }
    }
}
...
__global__ void gpu_spmv_csr(int N, int *rowPtr, int *colIndex, float *values, float *vector, float *result)
{
    ...
    if (row < N)
    {
        ...
        for (int col = rowPtr[row]; col < rowPtr[row + 1]; ++col)
        {
            sum += values[col] * vector[colIndex[col]];
        }
        result[row] = sum;
    }
}
...
int main (int argc, char argv[]) {
    ...
    float *h_values   = (float *)malloc(sizeof(float) * N);
    int   *h_rowPtr   = (int *)malloc(sizeof(int) * (N + 1));
    int   *h_colIndex = (int *)malloc(sizeof(int) * N);
    for (int i = 0; i < N; i++) {
        h_vector[i] = 1.0 + i;
        h_values[i] = 1.0;
        h_rowPtr[i] = i;
    ...
    cudaMalloc(&d_rowPtr, sizeof(int) * (N + 1));
    ...
    cudaMemcpy(d_rowPtr, h_rowPtr, sizeof(int) * (N + 1), cudaMemcpyHostToDevice);
    ...
    cpu_spmv_csr(N, h_rowPtr, h_colIndex, h_values, h_vector, cpu_result);
    ...
    gpu_spmv_csr<<<blocksPerBlock, threadsPerBlock>>>(N, d_rowPtr, d_colIndex, d_values, d_vector, d_result);
